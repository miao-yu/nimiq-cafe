...
export function BookingAvailable({ title, subheader, chart, sx, ...other }: Props) {
  ...
  const chartSeries = (chart.series.filter((i) => i.label === 'Current Supply')[0].value / total) * 100;
    ...
    plotOptions: {
      radialBar: {
        ...
        dataLabels: {
          name: { offsetY: -12 },
          value: { offsetY: 6 },
          total: { label: 'Max. Supply', formatter: () => `${fShortenNumber(total).toUpperCase()} $NIM` },
        },
      },
    },
    ...chart.options,
          ...
          >
            <Box
              sx={{
                ...
                ...(item.label === 'Current Supply' && { bgcolor: chartColors[1] }),
              }}
            />
            <Box sx={{ color: 'text.secondary', flexGrow: 1 }}>{item.label}</Box>
            {fShortenNumber(item.value).toUpperCase()} $NIM
          </Box>
        ))}
      </Box>
    </Card>
