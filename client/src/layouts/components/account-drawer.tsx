...
export function AccountDrawer({ user, sx, ...other }: AccountDrawerProps) {
  let name = '';
  let avatar = '';
  let link = '';
  ...
  if (user) {
    ...
    name = addressInfo ? addressInfo.name : fShortenString(user.address);
    avatar = addressInfo && addressInfo.logo ? addressInfo.logo : `https://v2.nimiqwatch.com/api/v1/iqon/${user.address.replaceAll(' ', '+')}`;
    link = `/wallet/${user.address}`;
  }
