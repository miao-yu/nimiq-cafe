...
export const _appInstalled = ['Germany', 'England', 'France', 'Korean', 'USA', 'England', 'France', 'Korean', 'USA', 'USA'].map(
  (country, index) => ({
    id: _mock.id(index),
    countryName: country,
    ...
    countryCode: ['de', 'gb', 'fr', 'kr', 'us', 'gb', 'fr', 'kr', 'us', 'us'][index],
  })
);
...
export const _poolStakers = Array.from({ length: 3 }, (_, index) => {
  const address = ['NQ62 8QT3 CUJ6 FXUT YVAY 0R5C KU0T H1F9 EC01', 'NQ74 C9KH 191S GSXS 62CU 6H8C UFF1 5U7J 01XX', 'NQ32 PRX5 NF6K MEC3 0033 DV4F AX4Q J96B 3SY5'][index];
  const addressLink = ['https://nimiq.watch/#NQ628QT3CUJ6FXUTYVAY0R5CKU0TH1F9EC01', 'https://nimiq.watch/#NQ74C9KH191SGSXS62CU6H8CUFF15U7J01XX', 'https://nimiq.watch/#NQ32PRX5NF6KMEC30033DV4FAX4QJ96B3SY5'][index];
  const shared = [33.94, 17.44, 12.00][index];
  const stakedNIM = [78000000, 40069999, 27583182][index];
  const pendingPayout = [0, 4.83147, 0.80064][index];
  const payoutType = ['Payout', 'Restake', 'Restake'][index];
  const avatar = ['https://v2.nimiqwatch.com/api/v1/iqon/NQ62+8QT3+CUJ6+FXUT+YVAY+0R5C+KU0T+H1F9+EC01', 'https://v2.nimiqwatch.com/api/v1/iqon/NQ74+C9KH+191S+GSXS+62CU+6H8C+UFF1+5U7J+01XX', 'https://v2.nimiqwatch.com/api/v1/iqon/NQ32+PRX5+NF6K+MEC3+0033+DV4F+AX4Q+J96B+3SY5'][index];
...
export const _ecommerceBestSalesman = Array.from({ length: 10 }, (_, index) => {
  const category = ['CAP', 'Branded shoes', 'Headphone', 'Cell phone', 'Earings', 'Branded shoes', 'Headphone', 'Cell phone', 'Earings', 'Earings'][index];
